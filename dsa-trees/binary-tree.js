...
class BinaryTree {
  ...
  nextLarger(lowerBound) {
    ...
    while (queue.length) {
      ...
      let higherThanLowerNound = currentVal > lowerBound
      let shouldReassignClosest = currentVal < closest || closest === null

      if (higherThanLowerNound && shouldReassignClosest) {
        closest = currentVal
      }
  ...
  static deserialize() {
    ...
    function buildTree() {
      // building a tree starting from the begining of the array
      if (values.length) {
        const currentVal = values.shift()

        if(currentVal === "#") return null

        // rememeber to convert values back ito numbers
  ...
  lowestCommonAncestor(node1, node2, currentNode=this.root) {
    // base case1: emtpy tree
    if (currentNode === null) return null

    // base case2: root is one of the target nodes
    ...
    // recursibley search the right sub-tree
